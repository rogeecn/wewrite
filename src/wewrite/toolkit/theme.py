...
def _default_themes_dir() -> str:
    """Return the built-in themes/ directory relative to this file."""
    return str(Path(__file__).parent / "themes")


def _theme_search_dirs(themes_dir: str = None) -> list[str]:
    """Theme lookup order: explicit dir > user themes ($WEWRITE_HOME/themes) > built-in."""
    if themes_dir is not None:
        return [themes_dir]
    from ..paths import user_themes_dir
    return [str(user_themes_dir()), _default_themes_dir()]
...
def load_theme(name: str, themes_dir: str = None) -> Theme:
    ...
    """
    search = _theme_search_dirs(themes_dir)
    theme_path = None
    for d in search:
        candidate = os.path.join(d, f"{name}.yaml")
        if os.path.exists(candidate):
            theme_path = candidate
            break
    if theme_path is None:
        raise FileNotFoundError(f"Theme file not found: {name}.yaml (searched: {', '.join(search)})")
...
def list_themes(themes_dir: str = None) -> list[str]:
    ...
    """
    names = set()
    for d in _theme_search_dirs(themes_dir):
