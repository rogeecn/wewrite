...
Usage:
    wewrite learn-theme <url>          # fetch + analyse live article
    wewrite learn-theme --file <path>  # analyse a saved HTML file
"""
...
def _load_template_css() -> str:
    """Load the base_css from the professional-clean template theme (built-in)."""
    from ..toolkit.theme import _theme_search_dirs
    for d in _theme_search_dirs():
        template_path = Path(d) / f"{TEMPLATE_THEME}.yaml"
        if template_path.exists():
            with open(template_path, encoding="utf-8") as fh:
                data = yaml.safe_load(fh)
            return data["base_css"]
    raise FileNotFoundError(f"template theme not found: {TEMPLATE_THEME}.yaml")
...
def main():
    ...
    parser.add_argument("--output-dir", default=None, help="Output directory (default: $WEWRITE_HOME/themes/)")
    ...
    print(f"Use it:  wewrite preview article.md --theme {args.name}")
